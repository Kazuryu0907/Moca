...
import { SocketComm } from './socket_communication';

export class SetPointModule {
  // GameScoreはsetPointの方の数字
  matchingScore = { blue: 0, orange: 0 };
  gameScore = { blue: 0, orange: 0 };
  preMatchingTeams: { blue: string | undefined; orange: string | undefined } = {
    blue: '',
    orange: ''
  };
  ...
  create_cmd_function(ws: SocketComm) {
    const func: WsCmdFuncType = (input) => {
      ...
      if (cmd === 'teamNames') {
        const teamNamesData: { blue: string; orange: string; matchId: string } =
          data;
        this.matchId = teamNamesData.matchId;
        this.matchingTeams = {
          blue: teamNamesData.blue,
          orange: teamNamesData.orange
        };
        // スコアリセット
        this.resetScore();
        // チーム名が違うプラベに入ったとき
        ...
        const winnerTeam =
          this.matchingScore.blue > this.matchingScore.orange
            ? 'blue'
            : 'orange';
  ...
  create_onConnection_function() {
    const func: WsOnConnectionType = (ws, path) => {
      ...
      ws.send(
        JSON.stringify({ cmd: 'currentScore', data: this.matchingScore })
      );
  ...
  private is_same_teams() {
    ...
    return (
      this.matchingTeams.blue === this.preMatchingTeams.blue &&
