...
import { ErrorHandleType, handle_error2, handle_error_async2, return_error } from "../common/handle_error";
...
export class New_start {
  ...
  private read_sheet_credential(full_sheet_credential_path: string): ErrorHandleType<sheet_credential_type> {
    ...
    // ファイルなかったら作る
    if (!existsSync(full_sheet_credential_path)) {
      writeFileSync(full_sheet_credential_path, JSON.stringify({ client_email: "", private_key: "" }));
    }
  ...
  private read_service_id_config(full_service_id_path: string): ServiceIDConfig {
    ...
    const service_id_config: ServiceIDConfig = {
      sheet_id: json.sheet_id,
      drive_id: json.drive_id,
      download_directory: json.download_directory,
    };
