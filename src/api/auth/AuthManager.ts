...
import {
  ...
  handle_error_async2,
  return_error
} from '../../common/handle_error';
import { AuthStatusMessage, AuthStatusMessageType } from '../../common/types';
import { DriveCredentialType, DriveService } from '../google_drive';
import { SheetService } from '../google_spreadsheet';
import {
  AuthConfig,
  AuthConfigSchema,
  AuthConfigTemplate,
...
export class AuthManager {
  ...
  async authenticate(): Promise<AuthResult> {
    ...
    try {
      ...
      const [credentials, credentialsError] =
        await this.loadAndValidateCredentials();
      if (credentialsError) {
        this.sendStatusMessage(
          'error',
          'credentials',
        ...
        return {
          success: false,
          errors: [credentialsError.error_message],
          config: null
        };
      }
      ...
      if (configError) {
        this.sendStatusMessage(
          ...
          'config',
          '設定ファイルの読み込みに失敗しました',
          [configError.error_message]
        );
        return {
          success: false,
          errors: [configError.error_message],
          config: null
        };
      }
      ...
      if (sheetError) {
        ...
        this.sendStatusMessage(
          ...
          [sheetError.error_message]
        );
      } else {
        this.sendStatusMessage(
          'success',
          'sheets',
          'Google Sheets認証が完了しました'
        );
      }
      ...
      if (driveError) {
        ...
        this.sendStatusMessage(
          ...
          [driveError.error_message]
        );
      } else {
        this.sendStatusMessage(
          'success',
          'drive',
          'Google Drive認証が完了しました'
        );
      }
  ...
  private async loadAndValidateCredentials(): Promise<
    ErrorHandleType<Credentials>
  > {
    try {
      if (!existsSync(this.credentialsPath)) {
        console.log(
          '📝 認証情報ファイルが見つかりません。テンプレートを作成します...'
        );
        await this.createCredentialsTemplate();
        return return_error(
          '認証情報ファイルを作成しました。Google Service Accountの認証情報を設定してください。'
        );
      }
  ...
  private async loadAndValidateConfig(): Promise<ErrorHandleType<AuthConfig>> {
    try {
      if (!existsSync(this.configPath)) {
        console.log(
          '📝 設定ファイルが見つかりません。テンプレートを作成します...'
        );
        await this.createConfigTemplate();
        return return_error(
          '設定ファイルを作成しました。認証情報とサービスIDを設定してください。'
        );
      }
  ...
  private async authenticateSheets(
    credentials: Credentials
  ): Promise<ErrorHandleType<void>> {
    try {
      const [, error] = await handle_error_async2(
        this.sheetService.auth(credentials)
      );
      if (error) {
